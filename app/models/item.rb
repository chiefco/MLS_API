  ...
  def self.get_meets(user,value=nil)
    ...
    unless value.nil?
      user.items.undeleted.each do |f|
        ...
        @meets_values=@meets_values.merge({f.id=>{:name=>f.name,:id=>f._id,:description=>f.description, :meet_id => "#{test}", :location_details=>location_values,:item_date=>f.item_date, :created_at=>f.created_time,:updated_at=>f.updated_time,:pages=>get_pages(f),:shares=>get_shares(f),:audio=>f.attachment.nil? ? 'nil' : f.attachment.file}})
      end
