...
class V1::SessionsController < Devise::SessionsController
  ...
  def perform_synchronisation(user)
    ...
    get_deleted_notes
   respond_to do |format|
    format.json{render :json =>success.merge(:synced_ids => @synched_meets, :bookmarked_ids => @bookmarked_ids, :deleted_notes => @deleted_notes, :comments => @comments.flatten, :ipad_ids =>@ipad_ids.uniq, :synched_page_ids => @ipad_page_ids.uniq, :synched_pages => @synched_pages, :share_ids => @share_ids, :shared_hashes => @synched_hash, :task_ids => @task_ids, :task_hashes => @synched_tasks, :meets => params[:user][0][:status]=="true" ? get_meets(true) : get_meets(nil), :other_users => CommunityUser.other_users(@user._id), :locations=>@user.locations.serializable_hash(:only=>[:_id,:name], :methods=>[:latitude_val,:longitude_val] ))}
   end
  end
...
def create_or_update_pages(pages,value=nil)
  ...
  def remove_from_community(community)
    ...
    #~ item = attachment.attachable.item unless attachment.nil? && attachment.attachable_type =="User"
