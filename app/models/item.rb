  ...
  def self.get_meets(user,value=nil)
    ...
    unless value.nil?
      user.items.undeleted.each do |f|
        @meets<<f._id.to_s
        @meets_values=@meets_values.merge({f.id=>{:name=>f.name,:id=>f._id,:description=>f.description,:location_name=>f.location_name.downcase,:item_date=>f.item_date,:location_name=>f[:location_name],:created_at=>f.created_time,:updated_at=>f.updated_time,:pages=>get_pages(f),:shares=>get_shares(f)}})
      end
