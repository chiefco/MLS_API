class V1::AttachmentsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.json  { render :json => { :attachments=>@attachments.to_json(:only=>[:_id, :file_name, :file_type, :size, :user_id, :folder_id, :content_type,:file,:created_at], :methods => [:user_name, :has_revision, :is_shared]).parse ,:total=>@count }.to_success }
  ...
  # Public: Returns storage data of the current user
  ...
  def get_user_storage
    if @current_user
      ...
      respond_to do |format|
        format.json  { render :json => { :size => size, :percentage => percentage, :subscription_type => subscription_type}.to_success }
      end      
    end
  end    
