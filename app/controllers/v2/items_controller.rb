      ...
      if  @item.status == true
          if @current_user._id == @item.user_id
            @authorised_item = true
              ...
              is_shared = @item.shares.map(&:community).map(&:community_users)
              res =  is_shared[0].where(:user_id => @current_user._id).first rescue nil
              if res
                 @authorised_item = true
              else
